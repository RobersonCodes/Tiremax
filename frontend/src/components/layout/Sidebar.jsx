...
import { LayoutDashboard, Wrench, Users, Package, DollarSign, BarChart3, Settings, X, ChevronRight, ShoppingCart, ShieldCheck } from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 p-4 border-b border-white/[0.07]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-lg text-white uppercase leading-none">{settings?.name || 'TireMax'}</p>
            <p className="text-[10px] text-white/30 uppercase tracking-widest truncate max-w-[120px]">Borracharia</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        ...
        {/* Link Super Admin — só para SUPER_ADMIN */}
      ...
      {settings?.plan === 'TRIAL' && settings?.trialEndsAt && (
        <div className="mx-3 mb-2 rounded-xl bg-yellow-400/10 border border-yellow-400/20 p-3">
          <p className="text-yellow-400 text-xs font-bold">🎁 Período de Teste</p>
      ...
      <div className="m-3 rounded-xl overflow-hidden border border-yellow-400/20">
        <div className="bg-[#151500] p-3">
          ...
          <a href={`https://wa.me/${whatsappNum}`} target="_blank" rel="noreferrer"
            className="w-full bg-yellow-400 text-black font-bold text-xs uppercase tracking-wide py-2 rounded-lg flex items-center justify-center gap-1.5">
            💬 Agendar Agora
          </a>
        </div>
      </div>
