...
export default function Header({ onMenuClick }) {
  ...
  return (
    <header className="h-[60px] shrink-0 bg-[#131313] border-b border-white/[0.07] flex items-center px-4 gap-3 sticky top-0 z-10">
      ...
      <div className="flex-1 max-w-md hidden md:flex items-center gap-2 bg-[#1a1a1a] border border-white/[0.07] rounded-lg px-3 py-2 group focus-within:border-yellow-400/40 transition-all">
        <Search size={15} className="text-white/25 group-focus-within:text-yellow-400 transition-colors shrink-0" />
        <input type="text" placeholder="Buscar cliente, serviço ou produto..."
      ...
      <div className="relative">
        <button onClick={() => setDropdownOpen(!dropdownOpen)}
          ...
          <div className="w-8 h-8 rounded-full flex items-center justify-center"
            ...
            <span className="text-black text-xs font-black font-display">{user?.name?.charAt(0).toUpperCase()}</span>
          </div>
          ...
          {dropdownOpen && (
            <>
              ...
              <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }}
                ...
                className="absolute right-0 top-full mt-2 w-52 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-card overflow-hidden z-20">
